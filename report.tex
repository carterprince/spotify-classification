...
\large Term Project: Data Science Application}
\author{
\textbf{Group 8} \\
Robera Abajobir (XGBoost) \\
Sanghyun An (Gaussian NB) \\
Austin Bell (KNN) \\
Carter Prince (MLP) \\
Anvita Yerramsetty (Random Forest) \\
Tyler Varma (Logistic Regression)
}
...
The Pearson correlation matrix (Figure \ref{fig:corr}) indicates multicollinearity among specific audio features. A correlation coefficient of $0.82$ was observed between \textbf{Energy} and \textbf{Loudness}, while \textbf{Acousticness} and \textbf{Energy} exhibited a negative correlation of $-0.74$. These correlations suggest that models assuming feature independence, such as Naive Bayes, may experience performance degradation.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{fig2_correlation.png}
    \caption{Feature Correlation Matrix indicating strong relationships between Energy, Loudness, and Acousticness.}
    \label{fig:corr}
\end{figure}
...
Inference speed is a critical factor for deployment. Figure \ref{fig:efficiency} plots Accuracy against Test Time (Log Scale). 

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{fig5_efficiency.png}
    \caption{Efficiency Frontier: Accuracy vs. Prediction Speed.}
    \label{fig:efficiency}
\end{figure}

\begin{itemize}
    \item \textbf{XGBoost} demonstrated the most favorable trade-off, providing the highest accuracy with low inference latency.
    \item \textbf{KNN} exhibited poor scalability. While its accuracy was comparable to Logistic Regression, the computational cost of calculating distances in high-dimensional space resulted in significantly slower inference times.
    \item \textbf{Gaussian NB} was the fastest model but yielded the lowest accuracy, likely due to the violation of the feature independence assumption.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Dimensionality Reduction:} For distance-based models such as KNN, applying Principal Component Analysis (PCA) or manually removing features with low separability (such as \texttt{key} and \texttt{mode}) may reduce noise and computational cost.
\end{enumerate}
