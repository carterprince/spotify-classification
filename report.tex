...
\begin{itemize}
    ...
    % Using raggedright to prevent text overflow with texttt
    {\raggedright
    \begin{itemize}
...
\subsection{Removal of Identifiers}
The raw dataset contained metadata fields including \texttt{artist\_name}, \texttt{track\_name}, and \texttt{track\_id}. While these fields are highly predictive of genre (e.g., a specific artist usually produces one genre), they do not generalize to unseen artists or new tracks. To ensure the model learned acoustic characteristics rather than memorizing specific catalog entries, these high-cardinality identifiers were removed prior to training.

\subsection{Retention of Correlated Features}
As noted in the Visual Examination, \texttt{energy} and \texttt{loudness} exhibited high correlation ($r=0.82$). Standard procedure in linear modeling often involves removing one of these to reduce multicollinearity. However, we chose to retain both for the following reasons:
\begin{itemize}
    \item \textbf{Tree-Based Robustness:} Our primary models (XGBoost, Random Forest) utilize embedded feature selection. They inherently select the most discriminative feature at each split, rendering manual removal unnecessary.
    \item \textbf{Interaction Effects:} Deep learning models (MLP) can often exploit subtle differences between correlated features (e.g., a track that is loud but low energy might indicate a specific sub-genre like drone metal) that would be lost if one feature were dropped.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{slide8_knn_distance.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{slide10_rf_complexity.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{slide11_xgb_learning_rate.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{slide11_xgb_iterations.png}
    \caption{XGBoost Accuracy vs. Iterations.}
    \label{fig:xgb}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{best_xgboost_cm.png}
    \caption{Confusion Matrix for the best XGB trial.}
    \label{fig:corr}
\end{figure}
...
\begin{enumerate}
    \item \textbf{Richer Data:} To exceed the observed accuracy ceiling of $\approx$55\%, future iterations should incorporate Convolutional Neural Networks (CNNs) on raw audio or perhaps a pretrained audio embedding model rather than relying solely on pre-calculated summary statistics.
    \item \textbf{Hierarchical Classification:} Given the taxonomical nature of music (e.g., ``Rock'' encompasses ``J-Rock''), a hierarchical model that predicts the super-genre before refining the specific sub-genre could improve performance.
...
\url{https://github.com/carterprince/spotify-classification}
